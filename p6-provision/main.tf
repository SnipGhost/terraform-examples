...
# В учебных целях. Для таких задач лучше использовать user_data - это надежнее
resource "null_resource" "provision" {
  count = 1

  depends_on = [vkcs_compute_instance.compute]

  connection {
    type        = "ssh"
    host        = vkcs_compute_instance.compute[count.index].access_ip_v4
    user        = "ubuntu"
    private_key = vkcs_compute_keypair.default_key_pair.private_key
    timeout     = "5m"
  }

  provisioner "remote-exec" {
    inline = [
      "echo '<h1>Hello from Terraform provisioner (vm ${count.index + 1})</h1>' | sudo tee /var/www/html/test.html",
    ]
  }
}
